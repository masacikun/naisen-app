...
export default async function FaxPage({
  ...
  searchParams: Promise<{ from?: string; to?: string; status?: string; cat?: string; trash?: string }>
}) {
  ...
  let q = supabaseServer
    ...
    .select('id, received_at, from_number, pages, pdf_filename, status, category, memo, drive_url, deleted_at')
    .order('received_at', { ascending: false })
    .limit(500)

  // 削除済み（ゴミ箱）: 削除日から30日後に自動で完全削除される（purge-fax.mjs）
  if (trash) q = q.not('deleted_at', 'is', null)
  ...
  return (
    <FaxClient
      rows={rows}
      canEdit={canEdit}
      filters={{ from: sp.from ?? '', to: sp.to ?? '', status: sp.status ?? '', cat: sp.cat ?? '', trash }}
    />
  )
}
