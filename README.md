...
## CDR取込（接続層・2026-07-14 フェーズ2検証済み／cron は go-live 時に有効化）
...
- cron（**未登録・go-live 時に有効化**）: `8,23,38,53 * * * * /var/www/naisen-app/scripts/pull_cdr.sh >> /var/www/naisen-app/logs/cdr-pull.log 2>&1`
- **go-live 手順**: ① `.env.local` の `CDR_CUTOFF_AT` を実運用開始日時（JST・'YYYY-MM-DD HH:MM:SS'）に更新 → ② 上記 cron 行を `crontab -e` で登録 → ③ 最初の2巡でログ（logs/cdr-pull.log）と `naisen_calls` 件数を確認し、**再実行で重複0（冪等）を実測**（2巡目の UPSERT 後に件数が増えないこと）
...
- **`CDR_CUTOFF_AT` は go-live 日時に更新すること**（現在は仮置き 2026-08-01 00:00:00 JST。これ以前＝試験通話は台帳に入らない）。実cdr26列は実物照合済み（想定との差異は23列目 dst_cnam のみ・未使用）
